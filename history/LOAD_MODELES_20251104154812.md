# 🔄 Chargement des Modèles depuis Bubble.io

## 🎯 Vue d'ensemble

Le système de chargement des modèles permet de **synchroniser automatiquement** les modèles personnalisés créés dans l'application avec Bubble.io. Les modèles sont chargés au démarrage de l'application et peuvent être rechargés manuellement.

---

## 📡 Endpoint Bubble.io

### Configuration

| Mode | Endpoint |
|------|----------|
| **Test** | `https://checkeasy-57905.bubbleapps.io/version-test/api/1.1/wf/apireturnmodeleforapp` |
| **Production** | `https://checkeasy-57905.bubbleapps.io/api/1.1/wf/apireturnmodeleforapp` |

### Requête

- **Méthode** : `GET`
- **Paramètre** : `conciergerie` (ID de la conciergerie)
- **Exemple** : `?conciergerie=1730741276842x778024514623373300`

### Réponse

```json
{
  "status": "success",
  "response": {
    "data": [
      {
        "_id": "1762265543692x203300711359588160",
        "nom": "voyageur",
        "Type Parcours": "Voyageur",
        "TypePhoto": "À l'arrivée et à la sortie",
        "Created Date": 1762265543701,
        "Modified Date": 1762265543703,
        "conciergerie": "1730741276842x778024514623373300",
        "Data Template parcour": {
          "body_raw_text": "{\"conciergerieID\":\"...\",\"userID\":\"...\",\"modele\":{...}}",
          "_api_c2_returned_an_error": false
        }
      }
    ]
  }
}
```

---

## 🔧 Parsing du `body_raw_text`

Le champ `Data Template parcour.body_raw_text` contient un **JSON stringifié** avec la structure suivante :

```json
{
  "conciergerieID": "1730741276842x778024514623373300",
  "userID": "1730741188020x554510837711264200",
  "modele": {
    "id": "custom-1762265537170",
    "nom": "voyageur",
    "type": "voyageur",
    "etatLieuxMoment": "arrivee-sortie",
    "pieces": [
      {
        "id": "piece-1762265537170-Cuisine",
        "nom": "Cuisine",
        "tachesDisponibles": [...],
        "tachesSelectionnees": [...]
      }
    ],
    "questionsChecklist": [],
    "createdAt": "2025-11-04T14:12:17.170Z",
    "updatedAt": "2025-11-04T14:12:17.170Z"
  }
}
```

**Extraction** : On parse le JSON et on extrait l'objet `modele` qui correspond à l'interface `ParcoursModele`.

---

## 📦 Fonctions utilitaires

### `loadModelesFromBubble()`

Charge les modèles depuis Bubble.io.

```typescript
const modeles = await loadModelesFromBubble(
  "1730741276842x778024514623373300", // conciergerieID
  true // isTestMode
);
```

**Retour** : `Promise<ParcoursModele[]>`

**Logs** :
```
📥 Chargement des modèles depuis Bubble.io...
   Endpoint: https://checkeasy-57905.bubbleapps.io/version-test/api/1.1/wf/apireturnmodeleforapp
   Conciergerie: 1730741276842x778024514623373300
   Mode: TEST
   ✅ 2 modèle(s) trouvé(s)
   ✅ Modèle chargé: "voyageur" (voyageur)
      - Pièces: 1
      - Questions: 0
      - État des lieux: arrivee-sortie
   ✅ Modèle chargé: "Test modele 2" (menage)
      - Pièces: 3
      - Questions: 3
      - État des lieux: arrivee-sortie

✅ 2 modèle(s) chargé(s) avec succès
```

---

### `loadAndMergeModeles()`

Charge les modèles depuis Bubble.io et les fusionne avec les modèles locaux.

```typescript
const mergedModeles = await loadAndMergeModeles(
  "1730741276842x778024514623373300", // conciergerieID
  localModeles, // Modèles stockés localement
  true // isTestMode
);
```

**Logique de fusion** :
1. Charger les modèles depuis Bubble.io
2. Créer un Map des modèles Bubble par ID
3. Créer un Map des modèles locaux par ID
4. Ajouter tous les modèles Bubble (priorité)
5. Ajouter les modèles locaux qui ne sont pas dans Bubble

**Retour** : `Promise<ParcoursModele[]>`

**Logs** :
```
📊 Fusion des modèles:
   - Modèles Bubble: 2
   - Modèles locaux: 3
   - Modèles fusionnés: 4
```

---

### `saveModelesToLocalStorage()`

Sauvegarde les modèles dans le localStorage.

```typescript
saveModelesToLocalStorage(modeles);
```

**Logs** :
```
💾 2 modèle(s) sauvegardé(s) dans le localStorage
```

---

### `loadModelesFromLocalStorage()`

Charge les modèles depuis le localStorage.

```typescript
const modeles = loadModelesFromLocalStorage();
```

**Retour** : `ParcoursModele[]`

**Logs** :
```
💾 2 modèle(s) chargé(s) depuis le localStorage
```

---

## 🚀 Intégration dans l'application

### Chargement au démarrage

Dans `App.tsx`, un `useEffect` charge automatiquement les modèles au démarrage :

```typescript
useEffect(() => {
  const loadModeles = async () => {
    setIsLoadingModeles(true);
    try {
      console.log("🚀 Chargement des modèles au démarrage de l'application...");
      
      // Charger les modèles locaux
      const localModeles = loadModelesFromLocalStorage();
      
      // Charger et fusionner avec les modèles Bubble
      const mergedModeles = await loadAndMergeModeles(
        CONCIERGERIE_ID,
        localModeles,
        IS_TEST_MODE
      );
      
      // Mettre à jour l'état
      setCustomModeles(mergedModeles);
      
      // Sauvegarder dans le localStorage
      saveModelesToLocalStorage(mergedModeles);
      
      console.log("✅ Modèles chargés avec succès");
    } catch (error) {
      console.error("❌ Erreur lors du chargement des modèles:", error);
      toast({
        title: "Erreur de chargement",
        description: "Impossible de charger les modèles depuis Bubble.io. Utilisation des modèles locaux.",
        variant: "destructive",
      });
    } finally {
      setIsLoadingModeles(false);
    }
  };

  loadModeles();
}, []); // Exécuter une seule fois au démarrage
```

---

### Rechargement manuel

Un bouton "Recharger les modèles" permet de synchroniser manuellement :

```typescript
const handleReloadModeles = async () => {
  setIsLoadingModeles(true);
  try {
    console.log("🔄 Rechargement manuel des modèles...");
    
    const bubbleModeles = await loadModelesFromBubble(CONCIERGERIE_ID, IS_TEST_MODE);
    
    setCustomModeles(bubbleModeles);
    saveModelesToLocalStorage(bubbleModeles);
    
    toast({
      title: "Modèles rechargés !",
      description: `${bubbleModeles.length} modèle(s) chargé(s) depuis Bubble.io.`,
    });
  } catch (error) {
    console.error("❌ Erreur lors du rechargement des modèles:", error);
    toast({
      title: "Erreur de rechargement",
      description: "Impossible de recharger les modèles depuis Bubble.io.",
      variant: "destructive",
    });
  } finally {
    setIsLoadingModeles(false);
  }
};
```

**Interface** :
```tsx
<Button 
  onClick={handleReloadModeles} 
  size="lg" 
  variant="outline"
  disabled={isLoadingModeles}
>
  <RefreshCw className={`mr-2 h-4 w-4 ${isLoadingModeles ? 'animate-spin' : ''}`} />
  {isLoadingModeles ? 'Chargement...' : 'Recharger les modèles'}
</Button>
```

---

### Sauvegarde lors de la création

Lors de la création d'un nouveau modèle, il est automatiquement sauvegardé :

```typescript
const handleSaveCustomModele = async (modele: ParcoursModele) => {
  let updatedModeles: ParcoursModele[];

  if (editingModele) {
    // Mise à jour
    updatedModeles = customModeles.map(m => m.id === modele.id ? modele : m);
  } else {
    // Création
    updatedModeles = [...customModeles, modele];
    
    // Envoyer le webhook à Bubble.io
    await dispatchModeleWebhook(modele);
  }

  setCustomModeles(updatedModeles);
  
  // Sauvegarder dans le localStorage
  saveModelesToLocalStorage(updatedModeles);
};
```

---

## 🔍 Gestion des erreurs

### Erreur de chargement depuis Bubble.io

Si le chargement depuis Bubble.io échoue, l'application utilise les modèles locaux :

```typescript
try {
  const mergedModeles = await loadAndMergeModeles(...);
  setCustomModeles(mergedModeles);
} catch (error) {
  console.error("❌ Erreur lors du chargement des modèles:", error);
  toast({
    title: "Erreur de chargement",
    description: "Impossible de charger les modèles depuis Bubble.io. Utilisation des modèles locaux.",
    variant: "destructive",
  });
}
```

### Erreur de parsing

Si un modèle ne peut pas être parsé, il est ignoré :

```typescript
for (const item of data.response.data) {
  try {
    const parsed = JSON.parse(bodyRawText);
    const modele = parsed.modele;
    modeles.push(modele);
  } catch (parseError) {
    console.error(`❌ Erreur lors du parsing du modèle "${item.nom}":`, parseError);
    continue; // Ignorer ce modèle et continuer
  }
}
```

---

## 📊 Flux de données complet

```
┌─────────────────────────────────────────────────────────────┐
│                   Démarrage de l'application                │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          1. Charger les modèles depuis localStorage         │
│             loadModelesFromLocalStorage()                   │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          2. Charger les modèles depuis Bubble.io            │
│             loadModelesFromBubble()                         │
│             GET /apireturnmodeleforapp?conciergerie=...     │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          3. Parser le body_raw_text de chaque modèle        │
│             JSON.parse(body_raw_text)                       │
│             Extraire modele.modele                          │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          4. Fusionner avec les modèles locaux               │
│             loadAndMergeModeles()                           │
│             Priorité aux modèles Bubble                     │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          5. Mettre à jour l'état de l'application           │
│             setCustomModeles(mergedModeles)                 │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│          6. Sauvegarder dans le localStorage                │
│             saveModelesToLocalStorage(mergedModeles)        │
└─────────────────────────────────────────────────────────────┘
```

---

## ✅ Checklist de vérification

Lors du chargement des modèles, vérifiez que :

- [ ] Les logs affichent le nombre de modèles chargés depuis Bubble.io
- [ ] Les logs affichent le nombre de modèles fusionnés
- [ ] Les modèles sont correctement affichés dans l'interface
- [ ] Le bouton "Recharger les modèles" fonctionne
- [ ] Les modèles sont sauvegardés dans le localStorage
- [ ] En cas d'erreur, les modèles locaux sont utilisés

---

## 🐛 Débogage

### Vérifier les logs de chargement

Ouvrez la console du navigateur et cherchez :

```
🚀 Chargement des modèles au démarrage de l'application...
📥 Chargement des modèles depuis Bubble.io...
   ✅ 2 modèle(s) trouvé(s)
   ✅ Modèle chargé: "voyageur" (voyageur)
📊 Fusion des modèles:
   - Modèles Bubble: 2
   - Modèles locaux: 0
   - Modèles fusionnés: 2
💾 2 modèle(s) sauvegardé(s) dans le localStorage
✅ Modèles chargés avec succès
```

### Tester manuellement l'endpoint

```bash
curl "https://checkeasy-57905.bubbleapps.io/version-test/api/1.1/wf/apireturnmodeleforapp?conciergerie=1730741276842x778024514623373300"
```

### Vérifier le localStorage

Dans la console du navigateur :

```javascript
JSON.parse(localStorage.getItem('custom-modeles'))
```

---

## 📁 Fichiers impliqués

| Fichier | Rôle |
|---------|------|
| `src/config/bubbleEndpoints.ts` | Configuration centralisée des endpoints Bubble.io |
| `src/utils/loadModeles.ts` | Fonctions utilitaires pour charger les modèles |
| `src/App.tsx` | Intégration du chargement au démarrage |
| `src/types/modele.ts` | Interface `ParcoursModele` |

---

## 🚀 Prochaines étapes

- [ ] Ajouter un indicateur de synchronisation dans l'interface
- [ ] Implémenter un système de cache avec expiration
- [ ] Ajouter la gestion des conflits (modèle modifié localement et sur Bubble)
- [ ] Implémenter la suppression de modèles côté Bubble.io

