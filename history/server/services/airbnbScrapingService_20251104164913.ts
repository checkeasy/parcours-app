...
async function getCompleteData(extractionId: string): Promise<CollectedStreamData> {
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      console.log(`   Pièces: ${data.metadata?.total_rooms}`);
      console.log(`   Tâches: ${data.metadata?.total_tasks}`);
      console.log(`   Structure des données:`, JSON.stringify(data, null, 2).substring(0, 500));
      ...
      const collectedData: CollectedStreamData = {
        ...
        stats: {
          rooms_found: data.metadata.total_rooms,
          total_images: Object.values(data.rooms).reduce((sum: number, room: any) => sum + room.images_count, 0),
          ai_used: data.metadata.ai_tasks_included || true,
          extraction_method: 'rest'
        },
        rooms: data.rooms,
        tasks: Object.entries(data.rooms).reduce((acc: any, [roomType, roomData]: [string, any]) => {
          acc[roomType] = {
            total_tasks: roomData.ai_generated_tasks.length + roomData.preselected_tasks.length,
            ai_generated_tasks: roomData.ai_generated_tasks,
            preselected_tasks: roomData.preselected_tasks
          };
          return acc;
        }, {})
      };
