...
async function getCompleteData(extractionId: string): Promise<CollectedStreamData> {
  ...
  const maxAttempts = 60; // 60 tentatives max (3 minutes avec 3 secondes entre chaque)
  let attempts = 0;
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (statusData.error && statusData.error.includes('non trouvée')) {
        throw new Error(`⚠️ PROBLÈME DU SERVICE PYTHON: L'extraction ${extractionId} a disparu après ${attempts} tentatives (${attempts * 3}s). Le service Python semble supprimer les extractions en cours. Vérifiez les logs du service Python (port 5000) et redémarrez-le si nécessaire.`);
      }
