...
async function downloadAndConvertToBase64(imageUrl: string): Promise<string> {
  try {
    console.log(`      📥 Téléchargement: ${imageUrl.substring(0, 80)}...`);
    ...
    console.log(`      ✅ Converti: ${(buffer.length / 1024).toFixed(2)} KB`);

    return dataUrl;
  } catch (error: any) {
    console.error(`      ❌ Erreur téléchargement image: ${error.message}`);
    // Retourner une image placeholder ou l'URL originale en cas d'erreur
    return imageUrl;
  }
}
