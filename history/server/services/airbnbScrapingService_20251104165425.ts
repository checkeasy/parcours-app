...
async function getCompleteData(extractionId: string): Promise<CollectedStreamData> {
  ...
  const maxAttempts = 60; // 60 tentatives max (60 secondes avec 1 seconde entre chaque)
  let attempts = 0;
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 400) {
          // L'extraction n'est pas encore terminée
          console.log(`   ⏳ Tentative ${attempts}/${maxAttempts}: Extraction en cours...`);
      ...
      console.log(`✅ Données complètes récupérées après ${attempts} tentative(s)`);
      console.log(`   Structure des données:`, JSON.stringify(data, null, 2).substring(0, 1000));

      // Les données sont dans extraction_data
      const extractionData = data.extraction_data;
      const metadata = data.metadata;

      // Transformer les données au format attendu par le reste du code
      const collectedData: CollectedStreamData = {
        metadata: metadata,
        propertyInfo: extractionData.property_info,
        stats: {
          rooms_found: metadata.total_rooms,
          total_images: metadata.total_images || 0,
          ai_used: metadata.ai_tasks_included || true,
          extraction_method: extractionData.property_info.extraction_method || 'rest'
        },
        rooms: extractionData.rooms,
        tasks: Object.entries(extractionData.rooms).reduce((acc: any, [roomType, roomData]: [string, any]) => {
          // roomData est un tableau d'images, pas un objet avec ai_generated_tasks
          // On doit récupérer les tâches depuis data.tasks
          const tasksForRoom = data.tasks?.[roomType] || { ai_generated_tasks: [], preselected_tasks: [] };
          acc[roomType] = {
            total_tasks: tasksForRoom.ai_generated_tasks.length + tasksForRoom.preselected_tasks.length,
            ai_generated_tasks: tasksForRoom.ai_generated_tasks,
            preselected_tasks: tasksForRoom.preselected_tasks
          };
          return acc;
        }, {})
      };
      ...
      if (attempts >= maxAttempts) {
        throw new Error(`Timeout: extraction trop longue (>${maxAttempts} secondes)`);
      }
...
async function transformToParcoursFormat(
  ...
  const pieces = await Promise.all(
    Object.entries(data.rooms).map(async ([roomType, roomImages]) => {
      ...
      console.log(`\n   🏠 ${mappedRoomName}: ${tasks.length} tâches, ${images.length} photos`);
