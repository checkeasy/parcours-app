...
import { ParcoursModele, TacheModele } from "@/types/modele";
...
export function AddLogementDialog({
  ...
  const getTasksForPiece = (pieceName: string, selectedModele: "menage" | "voyageur" | ParcoursModele): TacheModele[] => {
    if (typeof selectedModele === 'string') {
      // Predefined model - return empty array for now (tasks will be handled by backend)
      return [];
    } else {
  ...
  const dispatchWebhook = async (logementData: {
    nom: string;
    adresse?: string;
    parcoursType: "menage" | "voyageur";
    modele: "menage" | "voyageur" | ParcoursModele;
    ...
    try {
      ...
      // Generate a unique logement ID
      const logementId = `logement_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      // Prepare pieces with tasks included
      const piecesWithTasks = logementData.pieces.map(piece => ({
        ...piece,
      ...
      const payload = {
        conciergerieID: "conciergerie_demo", // Placeholder - should be from user context
        logementID: logementId,
        userID: "user_demo", // Placeholder - should be from user context
        ...
        pieces: piecesWithTasks,
        piecesPhotos: logementData.piecesPhotos,
      };
